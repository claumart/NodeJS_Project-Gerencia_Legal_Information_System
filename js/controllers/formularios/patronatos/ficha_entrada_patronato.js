...
app.controller("formCtrl", function($scope, $http, $window, utilities, limpieza, urlUtility) {
    ...
    LlenarCampos = ()=>{
       $http({
            method : "POST",
            url : $scope.serverUrl + "/modificacion/patronatos/obtener/formularioEntrada",
    ...
    DerivarNumExpediente = ()=>{
        $http({
            method : "POST",
            url : $scope.serverUrl + "/modificacion/patronatos/obtener/expedientes",
            data : {idFicha : $scope.urlParams.idFicha}
        }).then(function mySuccess(response) {
            ...
            $http({
                method : "POST",
                url : $scope.serverUrl + "/modificacion/patronatos/obtener/nombreComunidad",
                                        ...
                                        if(!isNaN($scope.folios_ins) && $scope.folios_ins > 0){
                                            if($scope.tipo_fecha == "actual"){
                                                ...
                                                if($scope.urlParams.mod == 1){
                                                    $http({
                                                        method : "POST",
                                                        url : $scope.serverUrl + "/modificacion/patronatos/actualizar/fichaEntrada/noAcumulado",
                                                        ...
                                                        if(i == numeroExpedientes){
                                                            if($scope.tipo_fecha == "actual"){
                                                                ...
                                                                if($scope.urlParams.mod == 1){
                                                                    $http({
                                                                        method : "POST",
                                                                        url : $scope.serverUrl + "/modificacion/patronatos/actualizar/fichaEntrada/acumulado",
                                                                        data : {interesado : interesadoValidado, apoderado : apoderadoLegal, 
                                                                            idProcedencia : $scope.procedencia_select.idDependencia, 
                                                                            numEmpleadoReceptor : $scope.empleado_receptor_select.numEmpleado, 
