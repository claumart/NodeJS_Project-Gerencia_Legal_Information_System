...
CREATE INDEX `PaginaDictamen_idDictamen_FK_idx` ON `PaginaDictamen` (`idDictamen` ASC) VISIBLE;

CREATE UNIQUE INDEX `urlPaginaDictamen_UNIQUE` ON `PaginaDictamen` (`urlPagina` ASC) VISIBLE;
...
CREATE UNIQUE INDEX `codigoCiudad_UNIQUE` ON `Ciudad` (`codigoCiudad` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `Comunidad` (
  ...
  CONSTRAINT `Comunidad_idCiudad_FK`
...
CREATE INDEX `Comunidad_idCiudad_FK_idx` ON `Comunidad` (`idCiudad` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `ExpedientePatronato` (
  ...
  `numExpedientePatronato` VARCHAR(35) NOT NULL,
  `folios` INT NOT NULL,
...
CREATE UNIQUE INDEX `urlPaginaArchivo_UNIQUE` ON `PaginaArchivoAdjunto` (`urlPagina` ASC) VISIBLE;

CREATE INDEX `PAA_idDictamen_FK_idx` ON `PaginaArchivoAdjunto` (`idDictamen` ASC) VISIBLE;
